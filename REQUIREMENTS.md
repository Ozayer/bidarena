...
- [x] A **room/projector display mode**: a large-screen, login-free view showing current player, current highest bid + bidder, live timer, recent bid ticker — designed for a shared screen in the room, separate from individual owner devices
  - [x] Admin-togglable **branded/themed display**: per-tournament logo uploads (tournament, club, sponsor) swap the room display into a bespoke themed look; toggle switches instantly back to the generic view — a one-off customization per tournament rather than a generic theming system
